...
// MODE register debugging functions
void checkMODERegister(const char* location) {
        uint8_t mode = Usb.regRd(rMODE);
        E_Notify(PSTR("\r\n[MODE CHECK] "), 0x80);
        Serial.print(location);
        E_Notify(PSTR(" - MODE register: 0x"), 0x80);
        print_hex(mode, 8);
        
        if (mode == 0xC9) {
                E_Notify(PSTR(" (HOST mode - OK)"), 0x80);
        } else if (mode == 0x0) {
                E_Notify(PSTR(" (PERIPHERAL mode - PROBLEM!)"), 0x80);
        } else {
                E_Notify(PSTR(" (Unknown mode)"), 0x80);
        }
}

void detailedRegisterDump(const char* location) {
        E_Notify(PSTR("\r\n[REGISTER DUMP] "), 0x80);
        Serial.print(location);
        
        uint8_t mode = Usb.regRd(rMODE);
        uint8_t pinctl = Usb.regRd(rPINCTL);
        uint8_t usbctl = Usb.regRd(rUSBCTL);
        uint8_t usbirq = Usb.regRd(rUSBIRQ);
        uint8_t revision = Usb.regRd(rREVISION);
        
        E_Notify(PSTR("\r\n  MODE: 0x"), 0x80);
        print_hex(mode, 8);
        E_Notify(PSTR("  PINCTL: 0x"), 0x80);
        print_hex(pinctl, 8);
        E_Notify(PSTR("  USBCTL: 0x"), 0x80);
        print_hex(usbctl, 8);
        E_Notify(PSTR("  USBIRQ: 0x"), 0x80);
        print_hex(usbirq, 8);
        E_Notify(PSTR("  REV: 0x"), 0x80);
        print_hex(revision, 8);
}
...
void setup() {
        ...
        E_Notify(PSTR("\r\n\n=== STARTING USB STACK INITIALIZATION ==="), 0x80);
        checkMODERegister("Before USB stack init");
        ...
        checkMODERegister("After USB stack init");
        detailedRegisterDump("After USB stack init");
...
void loop() {
        ...
        // Check MODE before USB task
        static uint8_t last_mode = 0xFF;  // Initialize to invalid value
        uint8_t current_mode = Usb.regRd(rMODE);
        if (current_mode != last_mode) {
                E_Notify(PSTR("\r\n[MODE CHANGE] Before Usb.Task() - MODE: 0x"), 0x80);
                print_hex(current_mode, 8);
                last_mode = current_mode;
        }
        
        Usb.Task();
        
        // Check MODE after USB task
        current_mode = Usb.regRd(rMODE);
        if (current_mode != last_mode) {
                E_Notify(PSTR("\r\n[MODE CHANGE] After Usb.Task() - MODE: 0x"), 0x80);
                print_hex(current_mode, 8);
                last_mode = current_mode;
        }
        ...
        if(usbstate != laststate) {
                ...
                case( USB_DETACHED_SUBSTATE_WAIT_FOR_DEVICE):
                        ...
                        checkMODERegister("USB_DETACHED_SUBSTATE_WAIT_FOR_DEVICE");
                        break;
                case( USB_ATTACHED_SUBSTATE_RESET_DEVICE):
                        E_Notify(PSTR("\r\nDevice connected. Resetting..."), 0x80);
                        checkMODERegister("USB_ATTACHED_SUBSTATE_RESET_DEVICE");
                        break;
                case( USB_ATTACHED_SUBSTATE_WAIT_SOF):
                        E_Notify(PSTR("\r\nReset complete. Waiting for the first SOF..."), 0x80);
                        checkMODERegister("USB_ATTACHED_SUBSTATE_WAIT_SOF");
                        break;
                case( USB_ATTACHED_SUBSTATE_GET_DEVICE_DESCRIPTOR_SIZE):
                        E_Notify(PSTR("\r\nSOF generation started. Enumerating device..."), 0x80);
                        checkMODERegister("USB_ATTACHED_SUBSTATE_GET_DEVICE_DESCRIPTOR_SIZE");
                        break;
                case( USB_STATE_ADDRESSING):
                        E_Notify(PSTR("\r\nSetting device address..."), 0x80);
                        checkMODERegister("USB_STATE_ADDRESSING");
                        break;
                                        case( USB_STATE_RUNNING):
                        E_Notify(PSTR("\r\nGetting device descriptor"), 0x80);
                        checkMODERegister("USB_STATE_RUNNING - Before getDevDescr");
                        rcode = Usb.getDevDescr(1, 0, sizeof (USB_DEVICE_DESCRIPTOR), (uint8_t*) & buf);
                        checkMODERegister("USB_STATE_RUNNING - After getDevDescr");

                        if(rcode) {
                                E_Notify(PSTR("\r\nError reading device descriptor. Error code "), 0x80);
                                print_hex(rcode, 8);
                                checkMODERegister("USB_STATE_RUNNING - After getDevDescr ERROR");
                                detailedRegisterDump("USB_STATE_RUNNING - After getDevDescr ERROR");
                        } else {
                                        /**/
